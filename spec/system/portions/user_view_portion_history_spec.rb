...
describe 'Usuário vê o histórico de uma porção' do
  it 'depois de editar a porção do prato várias vezes' do
    ...
    expect(current_path).to eq(establishment_dish_portion_historicals_path(establishment, dish, portion)) 
  ...
  it 'depois de editar a porção da bebida várias vezes' do
    ...
    expect(current_path).to eq(establishment_beverage_portion_historicals_path(establishment, beverage, portion)) 
    expect(page).to have_content('30.00')
    expect(page).to have_content('40.00')
  end
end